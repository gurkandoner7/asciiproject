...
import com.portal.asciiproject.databinding.AsciiContinueButtonBinding

class AsciiContinueButton @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr) {
    private var _binding: AsciiContinueButtonBinding? = null
    private val binding: AsciiContinueButtonBinding
        get() = _binding!!
    private val attributes =
        context.obtainStyledAttributes(attrs, R.styleable.AsciiContinueButton)

    init {
        _binding = AsciiContinueButtonBinding.inflate(LayoutInflater.from(context), this, true)
        binding.apply {
            continueButton.text =
                attributes.getString(R.styleable.AsciiContinueButton_android_text)

            attributes.getDimension(
                R.styleable.AsciiContinueButton_android_textSize,
                20f
            ).apply {
        ...
        if (attributes.getBoolean(R.styleable.AsciiContinueButton_setOutlinedButtonStyle, false))
            setOutlinedButtonStyle()


        if (attributes.getBoolean(R.styleable.AsciiContinueButton_setCancelButtonStyle, false))
    ...
    fun enableButton() {
        binding.continueButton.apply {
            isEnabled = true
            setBackgroundResource(R.drawable.bg_rounded_button)
        }
    }
