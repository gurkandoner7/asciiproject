...
import com.portal.asciiproject.databinding.AsciiTextViewBinding
import com.portal.asciiproject.utilities.extensions.setColoredSpan
import com.portal.asciiproject.utilities.extensions.withClickableSpan

class AsciiTextView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr) {
    private var _binding: AsciiTextViewBinding? = null
    private val binding: AsciiTextViewBinding
        get() = _binding!!

    private val attributes = context.obtainStyledAttributes(attrs, R.styleable.AsciiTextView)
    private val _textSize = attributes.getDimension(
        R.styleable.AsciiTextView_android_textSize,
        12F
    )
    private val textColor = attributes.getColor(
        R.styleable.AsciiTextView_android_textColor,
        ContextCompat.getColor(context, R.color.navyBlue)
    )
    private val _maxLines = attributes.getInt(R.styleable.AsciiTextView_android_maxLines, -1)
    private val _ellipsize = attributes.getInt(R.styleable.AsciiTextView_android_ellipsize, 0)

    init {
        _binding = AsciiTextViewBinding.inflate(LayoutInflater.from(context), this, true)
        initAttributes()
    }

    private fun initAttributes() {
        when (attributes.getInteger(R.styleable.AsciiTextView_textType, 0)) {
        ...
        binding.textView.text = attributes.getString(R.styleable.AsciiTextView_android_text)
        ...
        when (attributes.getInteger(R.styleable.AsciiTextView_textAlignment, 0)) {
